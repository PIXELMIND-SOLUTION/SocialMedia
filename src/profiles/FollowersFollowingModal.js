...
const FollowersFollowingModal = ({ isOpen, onClose, userId, initialTab = 'followers' }) => {
  ...
  const fetchFollowersFollowing = async () => {
    try {
      ...
      // Fetch following
      const followingResponse = await fetch(`http://31.97.206.144:5002/api/following/${userId}`);
  ...
  const fetchUserProfiles = async (users) => {
    const profilePromises = users.map(async (user) => {
      ...
      try {
        const response = await fetch(`http://31.97.206.144:5002/api/profiles/${user._id}`);
  ...
  const handleApproveRequest = async (requesterId) => {
    try {
      const response = await fetch(`http://31.97.206.144:5002/api/approve-request`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleRejectRequest = async (followerId) => {
    try {
      const response = await fetch(`http://31.97.206.144:5002/api/reject-request`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
